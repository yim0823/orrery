...
**하드 의존과 소프트 의존.** 관계에 `strength`가 있습니다. 기본값은 `hard`입니다. 소프트 간선은 **넘어가는 사건의 심각도를 '저하'까지로 깎습니다.**

```
하드 간선:  dependency_down  ──▶  dependency_down
소프트 간선: dependency_down  ──▶  dependency_degraded
             node_lost        ──▶  dependency_degraded
             dependency_degraded ─▶ 그대로 (이미 상한)
```

처음 구현에서는 소프트 간선이 '저하' 사건을 **완전히 흡수**하게 했습니다. 느려진 선택적 의존은 내 문제가 아니라는 논리였는데, 백테스트가 반박했습니다. 체크아웃이 느리면 그 앞의 웹 프론트도 느립니다. 흡수는 **놓침(MISS)** 2건을 만들었고, 놓침은 사람을 다치게 하는 오류입니다. 그래서 소프트 간선은 사건을 삼키지 않고 약화시키기만 합니다.
...
## 10. 백테스팅 — 이 지도가 맞는지 재는 법

`src/orrery/backtest/`

영향 범위를 계산할 수는 있지만, 그 답이 현실과 맞는지는 별개입니다. 백테스팅은 과거 장애를 다시 돌려 엔진을 채점합니다.

```
장애 기록 ──▶ 그 시점의 world ──▶ propagate ──▶ 예측 상태
                                                   │
                        관측된 실제 상태 ◀──────────┴──▶ 판정
```

### 두 가지 설계 결정이 결과를 좌우합니다

**침묵은 건강이 아닙니다.** 기록에 없는 개체는 **채점에서 제외**하며, 정상이었다고 치지 않습니다. 장애 때 우리가 아는 것은 누군가 확인한 것뿐입니다. 아무도 안 본 시스템을 정상으로 세면 정밀도가 공짜로 올라가고, 그 숫자로 남을 설득하게 됩니다. `assume_unlisted_up: true`는 기록이 정말 전 세계를 덮을 때만 켜십시오.

**심각도를 셉니다.** 저하였는데 사망이라고 예측한 것은 적중이 아닙니다.

| 판정 | 뜻 |
|---|---|
| `hit` | 맞췄고 심각도까지 맞음 |
| `correct_up` | 영향 없음을 맞춤 |
| `understated` | 저하라 했는데 사망 |
| `overstated` | 사망이라 했는데 저하 |
| `false_alarm` | 죽는다 했는데 멀쩡 |
| `MISS` | **괜찮다 했는데 죽음** — 가장 위험한 오류 |

### 지표

- **재현율(recall)** = 실제로 깨진 것 중 깨진다고 예측한 비율. 놓침을 잡는 지표
- **정밀도(precision)** = 깨진다고 한 것 중 실제로 깨진 비율. 과잉 경보를 잡는 지표
- **정확도(exact)** = 심각도까지 정확히 맞은 비율

**여러 장애를 합칠 때는 비율의 평균이 아니라 판정을 모아서 계산합니다.** 개체 1개짜리 장애와 40개짜리 장애를 같은 무게로 평균 내면 작은 장애가 점수를 좌우합니다.
...
그 숫자가 당신의 인프라에서 나오기 전까지 이 도구의 출력은 **참고용**이고, 그렇게 말해야 합니다. 검증되지 않은 영향 범위를 근거로 변경을 승인하는 것이 지금 가장 큰 오용 위험입니다.
